const ProdutoPedidoModel = require("../models/produto_pedidos.model.js");
  ...
  ProdutoPedidoModel.findById(req.params.id, (err, data) => {
    if (err) {
      if (err.type == "not_found") {
        res.status(404).send({
          message: "Produto_Pedido não encontrado. ID: " + req.params.id,
        });
      } else {
        res.status(500).send({
