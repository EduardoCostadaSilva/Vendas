...
ProdutoPedidoModel.findById = (id, result) => {
  sql.query(
    "SELECT * FROM produtos_pedidos WHERE idproduto_pedido= " + id,
    (err, res) => {
      if (err) {
        console.log("erro: ", err);
        result(null, err);
...
ProdutoPedidoModel.create = (produtos_pedidos, result) => {
  sql.query(
    "INSERT INTO produtos_pedidos SET ?",
    produtos_pedidos,
    (err, res) => {
      if (err) {
        console.log("Erro: ", err);
      ...
      result(null, { idproduto_pedido: res.insertId, ...produtos_pedidos });
    }
  );
};
...
ProdutoPedidoModel.updateById = (id, produtos_pedidos, result) => {
  sql.query(
    "UPDATE produtos_pedidos SET observacao = ?,produtos_idproduto = ?, pedidos_idpedido = ? WHERE idproduto_pedido= ?",
    [
      produtos_pedidos.observacao,
      produtos_pedidos.produtos_idproduto,
      produtos_pedidos.pedidos_idpedido,
      id,
    ],
    (err, res) => {
      if (err) {
        ...
        result(null, { idproduto_pedido: id, ...produtos_pedidos });
      }
    }
  );
};
...
ProdutoPedidoModel.remove = (id, result) => {
  sql.query(
    "DELETE FROM produtos_pedidos WHERE idproduto_pedido = ?",
    id,
    (err, res) => {
      if (err) {
        console.log("erro: ", err);
